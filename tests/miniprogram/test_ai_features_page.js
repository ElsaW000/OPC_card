...
async function testOnLoadPrewarmsRemoteSession() {
  ...
  ;[absolutePath, aiServicePath, userServicePath, apiConfigPath, settingsServicePath].forEach((moduleId) => {
...
async function testSendMessageShowsProgressiveFeedbackAndCapturesTrace() {
  ...
  timerHarness.run(2000)
  assert.strictEqual(ctx.data.loadingStatusText, '正在连接模型', '超过 2 秒后应提示连接模型')

  timerHarness.run(8000)
  assert.strictEqual(ctx.data.loadingStatusText, '响应较慢，可重试或先看本地建议', '超过 8 秒后应给出重试或降级提示')
